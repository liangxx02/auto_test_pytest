# 摘自网络，修改了部分代码 data = json.loads(data[0].decode(self.__encoding, 'ignore'), encoding=self.__encoding)
...
class dubbo:
    ...
    def do(self, command):
        ...
        try:
            data = data.split(b"\n")  # 基於python3修改
            data = json.loads(data[0].decode(self.__encoding, 'ignore'), encoding=self.__encoding)
